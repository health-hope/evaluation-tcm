<template>
    <div class="presentation">
        ...
        <div class="zy-content-main">
            <div class="zy_item_temp" v-for="(item,index) in reportContent" :key="index">
                ...
                <div class="pt-list" v-if="item.templateType == 'ZY_ZH_FB'">
                    <TheZyZhFb :tempData="item.template" :code="evaluationCode" @hook:mounted="onshow"/>
                </div>
                <!-- 中医介绍症状模板 -->
        ...
        <div class="btn-box" v-show="show">
            <div :style="{backgroundImage: 'url(jgy_button_bg_bai.png)'} | PUBLIC_PATH" class="btn btn-reval tap_events" @click="handleGoPage1()">再测一次</div>
            <div :style="{backgroundImage: 'url(jgy_button_bg_huang.png)'} | PUBLIC_PATH" class="btn btn-plan tap_events" @click="handleGoPage('/programme')">调理方案</div>
        </div>
...
let isTouchBack = true //是否点击了返回按钮
//无状态模版
...
export default {
    ...
    computed: {
        ...mapState(['evaluationId','evaluationCode'])
    },
    async created() {
        this.$loading.show();
        await this.searchId();
